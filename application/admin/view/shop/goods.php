...
<button class="btn button_btn btn-danger" type="button" id="del"><i class="fa fa-trash-o"></i>&nbsp;删除</button>
<span class="submenu"><a href="{:url('admin/shop/add_goods')}" class="btn button_btn bg-deep-blue" title="添加产品"><i class="fa  fa-edit"></i>&nbsp;添加商品</a></span>
<div class="search  clearfix">
<form action="" method="get">
 <label class="label_name">商品搜索：</label><input name="goods_name" id="goods_name" type="text"  class="form-control col-xs-6"/><i class="fa  fa-search"></i>&nbsp;
 <input type="submit" value="搜索" class="btn button_btn bg-deep-blue " ></input>
</form>
</div>
...
<div class="list_Exhibition margin-sx">
 ...
 <?php foreach($data as $val):?> 
   	<tr>
 <th width="30"><label><input type="checkbox" name="goods_id" value="<?=$val['goods_id']?>" ><span class="lbl"></span></label></th>
   <th width="100"><?= $val['goods_id']?></th>
   ...
   <th width="100">
    ...
    <a href="javascript:" id="stop" is_on_sale="0" goods_id="<?= $val['goods_id']?>"  class="btn bg-green operation_btn">上架</a>
    <?php }else{?>
    <a href="javascript:" id="stop" is_on_sale="1" goods_id="<?= $val['goods_id']?>" class="btn bg-green operation_btn">下架</a>
   <?php }?>
   </th>
   <th width="220"><a href="{:url('admin/shop/update_goods')}?goods_id=<?= $val['goods_id']?>" class="btn bg-deep-blue operation_btn">修改</a> 
     <a href="{:url('admin/shop/del_goods')}?goods_id=<?= $val['goods_id']?>" class="btn btn-danger operation_btn">删除</a> 
   </th>
   </tr>
 <?php endforeach;?> 
...
<span style="float: right; margin-right: 50%;"> {$page}</span>
...
<!-- <div class="add_Carousel_figure" id="add_Carousel_figure" style=" display:none">
  ...
  <div class="title_name"><i></i>添加轮播图</div>
    ...
    <script type="text/javascript" charset="utf-8" src="/jquery.1.12.js"></script>
 ...
 $(function() { 
	...
	// });
     $(".ace").click(function(){
     	...
     	// }
     	var ace=$(".ace").is(':checked');
     	 $("input[type='checkbox']").each(function() 
     	   {  
               this.checked = ace;       //循环赋值给每个复选框是否选中
            });
     })
    ...
    $("#del").click(function(){
       ...
       var goods_id=Array();
        for(k in obj){
        if(obj[k].checked)
            goods_id.push(obj[k].value);
    }
    
       $.ajax({
       	 url:"{:url('admin/shop/del_all')}",
       	 data:{"goods":goods_id},
       	 type:'post',
    ...
    //是否上架
   $("#stop").click(function(){
  	 var is_on_sale=$(this).attr("is_on_sale");
  	 var goods_id=$(this).attr("goods_id");
  	 $.ajax({
  	 	url:"{:url('admin/shop/update_display')}",
  	 	type:'post',
  	 	data:{'is_on_sale':is_on_sale,'goods_id':goods_id},
  	 	success:function(res)
  	 	{
  	 		if(res==1)
  	 		{
  	 		    ...
  	 		    {
  	 		    	$("#stop").text("下架");
                    $("#stop").attr("is_on_sale",'1');
  	 		    }else{
  	 		    	$("#stop").text("上架");
  	 		    	$("#stop").attr("is_on_sale",'0');
  	 		    }
  	 		}
  	 	}
  	 })
