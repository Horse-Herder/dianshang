...
<!--[if lt IE 9]>
  ...
  <![endif]-->
<body>
<form action="{:url('admin/user/access_add')}" method="post">
<div class="margin" id="page_style">
<div class=" add_Competence_style margin" id="add_Competence_style">
   <ul class="add_style">
    <li class="clearfix"><label class="label_name col-xs-1 col-lg-2">角色名称：</label><span class="col-xs-6"><input name="access_name" type="text"  class="col-xs-5"/></span></li>
     <li class="clearfix"><label class="col-xs-1 col-lg-2 label_name" for="form-field-1">权限描述：</label>
    <span class="col-xs-6"><textarea name="access_desc" class="form-control col-xs-10" id="form_textarea" placeholder="" ></textarea>
    </li>
    ...
    <div class="col-xs-6 admin_name clearfix">
      <?php foreach ($user_info as $key => $val): ?>
         <label class="middle"><input class="ace" value="<?=$val['user_id'] ?>" name="user_id[]" type="checkbox" id="id-disable-check"><span class="lbl"><?=$val['user_name'] ?></span></label>
      <?php endforeach ?>
  	</div></li>
   </ul>
</div>
              ...
              <dl class="Competence_name">   
                 ...
                 <dd class="permission_list clearfix">
                    <?php foreach ($val['child'] as $k => $v): ?>
                       <label class="middle"><input class="ace" value="<?= $v['url_id'] ?>" name="url_id[]" type="checkbox" id="id-disable-check"><span class="lbl"><?=$v['url_name'] ?></span></label>
                    <?php endforeach ?>
                 </dd>
              </dl>
         </div>
